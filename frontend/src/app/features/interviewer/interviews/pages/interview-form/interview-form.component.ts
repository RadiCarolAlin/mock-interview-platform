...
export class InterviewFormComponent implements OnInit {
  ...
  constructor(
    ...
    this.interviewForm = this.fb.nonNullable.group({
      ...
      type: [1, [Validators.required, Validators.pattern(/^[1-3]$/)]],
      level: [2, [Validators.required, Validators.pattern(/^[1-4]$/)]],
      ...
      durationMinutes: [
        ...
        [
          Validators.required,
          Validators.min(15),
          Validators.max(240), integer
        ]
      ],

      topics: ['', Validators.maxLength(1000)],
  ...
  submit(): void {
    if (this.loading || this.loadError || this.submitting || this.interviewForm.invalid) {
    ...
    if (!Number.isFinite(date.getTime())) {
      ...
      this.submitError = 'Enter a valid date and time.';
      return;
    }
